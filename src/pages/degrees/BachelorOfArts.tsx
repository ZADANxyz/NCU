...
const BachelorOfArts = () => {
  ...
  useEffect(() => {
    const loadCourses = async () => {
      try {
        const pdfs = await googleDriveService.fetchPdfsForDegree('bachelor');
        setCourses(pdfs);
      } catch (error) {
        console.error("Failed to load bachelor courses:", error);
      } finally {
        setLoading(false);
      }
    };
        ...
        <div className="w-full">
          ...
          <div className="space-y-4">
            {loading ? (
              ...
              courses.map((course) => (
                <Link key={course.id} to={`/degrees/bachelor-of-arts/${course.slug}`} className="block">
                  <Button
                    variant="outline"
